  ...
  return (
    <nav className='flex space-x-5 items-center px-5 h-16 border-b-2 lg:px-20'>
        ...
        <ul className='flex space-x-5'>
            {links.map(link => 
                <Link className={`${link.navlink === currentPath ? "text-slate-500" : "text-slate-700"} hover:text-slate-600 ease-in-out duration-300`} key={link.navlink} href={link.navlink}>
                    {link.label}
                </Link>
            )}
        </ul>
